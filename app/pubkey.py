...
"""Per-site public-key end-to-end encryption (``TSPEPK01``).

This is the real end-to-end layer: each connected site has an X25519
keypair. The **public** key lives on this server (and is handed to the TS
Pro client via ``/api/v1/ping``); the **private** key is shown to the
operator exactly once at site creation and is *never* stored here. The
client encrypts every backup *to the public key* before upload, so the
server only ever holds ciphertext it has no key for — even a fully
compromised server cannot read a backup. Only the operator, holding the
private key out-of-band, can decrypt at restore time.

This module must stay byte-for-byte compatible with TS Pro's
``app/pubkey.py``: the client encrypts with one, the operator decrypts
with the other. Change the envelope here and you must change it there.
...
generate an ephemeral keypair ``(e, E)``, compute the X25519 shared
secret ``s = X25519(e, R)``, derive a 32-byte AES key
``K = HKDF-SHA256(s, salt=E‖R, info='tspro-backup-e2ee-v1')``, then
stream-encrypt under AES-256-GCM. Binding ``E`` and ``R`` into the HKDF
salt ties the derived key to this exact (ephemeral, recipient) pair. To
decrypt, the holder of ``r`` recomputes ``s = X25519(r, E)`` and the same
``K``. The GCM tag covers the whole ciphertext; any tampering fails at
finalize.

Both encrypt and decrypt walk the input in 1 MiB blocks, so a multi-GB
archive costs O(1) memory — matching ``restenc`` / ``bundle_crypto``.
"""
...
class E2EEDecryptError(Exception):
    ...
    corrupted blob, or bad magic bytes."""
...
def generate_keypair() -> tuple[str, str]:
    ...
    strings. The private key is the secret the operator must keep."""
...
def fingerprint(public_key: str) -> str:
    """Short, stable, human-comparable id for a public key (first 16 hex
    of its SHA-256). Surfaced in both consoles so an operator can confirm
    the client is encrypting to the key the server actually holds."""
    try:
...
def decrypt_with_privkey(src_path: str, dst_path: str, private_key: str) -> None:
    ...
    Raises ``E2EEDecryptError`` on any failure (wrong key, magic
