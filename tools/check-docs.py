...
does not go out while it fails (docs/RELEASING.md).
  ...
  2. Every docs/*.md is listed in docs/README.md, so nothing becomes a file
     nobody will find.
  3. docs/ holds Markdown only; artwork belongs in app/static/img/.
  ...
  6. The newest CHANGELOG version equals __version__ in app/version.py.
  7. No Markdown file contains an em dash (docs/CONTRIBUTING.md, "Prose
     style"): a colon, a comma or a full stop instead.
...
def check_docs_dir() -> None:
    ...
    for path in docs.rglob("*"):
        if path.is_file() and path.suffix != ".md":
            fail(f"{path.relative_to(ROOT)}: docs/ holds Markdown only; artwork goes in app/static/img/")
...
def check_version(newest: str | None) -> None:
    ...
    m = re.search(r'^__version__\s*=\s*"([^"]+)"', path.read_text(), re.M) if path.exists() else None
    if not m:
...
def check_em_dashes(files: list[Path]) -> None:
    for path in files:
        for n, line in enumerate(path.read_text(encoding="utf-8").splitlines(), 1):
            if "\u2014" in line:
                fail(f"{path.relative_to(ROOT)}:{n}: an em dash; use a colon, a comma or a full stop")
